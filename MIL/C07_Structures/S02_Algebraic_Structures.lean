...
instance {α : Type*} [AddGroup₂ α] : Zero α := ⟨AddGroup₂.zero⟩

instance {α : Type*} [AddGroup₂ α] : Neg α := ⟨AddGroup₂.neg⟩
...
instance : AddGroup₂ Point where
  ...
  zero_add := by simp [Point.add, Point.zero, zero_add]
  add_zero := by simp [Point.add, Point.zero, add_zero]
  neg_add_cancel := by simp [Point.add, Point.zero, Point.neg]
